...
async function fetchOrders() {
    ...
    try {
        ...
        ordersList.innerHTML = `<div class="error">Failed to load orders. Make sure the services are running (Projection Service on port 8083).</div>`;
    }
}

function displayOrders(orders) {
    if (!orders || orders.length === 0) {
        ordersList.innerHTML = '<div class="empty-state">No orders found. Create your first order!</div>';
        return;
    }
    ...
    ordersList.innerHTML = sortedOrders.map(order => `
        <div class="order-card" data-id="${order.id}">
            ...
            <div class="order-details">
                ...
                <div class="order-detail">
                    <span class="order-detail-label">Created At</span>
...
async function createOrder(event) {
    ...
    try {
        ...
        showMessage(`Order created successfully! ID: ${result.id}`, 'success');
        ...
        showMessage('Failed to create order. Make sure the Order Service is running on port 8080.', 'error');
    }
}
